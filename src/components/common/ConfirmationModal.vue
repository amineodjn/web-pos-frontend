<template>
  <div v-if="modelValue" 
    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md relative shadow-xl dark:bg-dark-bg dark:text-white">
      <!-- Processing overlay -->
      <div v-if="isProcessing" class="absolute inset-0 bg-white bg-opacity-70 flex items-center justify-center z-10 dark:bg-gray-900 dark:bg-opacity-70">
        <div class="text-center">
          <SpinnerUI />
          <p class="text-gray-600 mt-4 dark:text-gray-300">{{ processingText }}</p>
        </div>
      </div>
      ...
      <div class="mb-6">
        <div class="flex items-center mb-4 text-red-600">
          <slot name="icon">
            <svg class="w-8 h-8 mr-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <div class="flex justify-end gap-3">
        ...
        <button @click="confirm"
          ...
          <span>{{ isProcessing ? processingText : confirmText }}</span>
        </button>
      </div>
    </div>
  </div>
