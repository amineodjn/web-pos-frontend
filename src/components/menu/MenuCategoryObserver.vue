<template>
  <div :id="`category-section-${category}`" ref="sectionRef" class="category-section mb-8">
    <slot></slot>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const getViewportConfig = (width = window.innerWidth): ViewportConfig => ({
    ...
    rootMargin:
      width <= 768 ? '-10% 0px' : width <= 1024 ? '-15% 0px -50% 0px' : '-10% 0px -65% 0px',
  });

  const emit = defineEmits<{
    (e: 'category-visible', categoryId: string): void;
  }>();
    ...
    observer = new IntersectionObserver(
      ([entry]) => {
        if (entry?.isIntersecting && entry.intersectionRatio > threshold) {
          emit('category-visible', props.category);
        }
      },
